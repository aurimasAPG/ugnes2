...
NPC bodies are generated by `CharacterRig` from a per-key proportion table
(silhouette + prop: Vesk apron, Orrel ledger, Coll gear charm). A new NPC gets the
default body automatically; a distinct silhouette is one new table row — which is a
C# edit, so if NPC #5 needs one, add the row *before* the Phase-2-style gate run,
or accept the default body (data-only stays true).
